...
*/
func (p *playlist) reverseSong() {
	if p.head == nil || p.head.next == nil {
		fmt.Println("Nil or Empty playlist")
		return
	}
	...
	for cursor = nil; p.head != nil; p.head = tmp {
		tmp = p.head.next
		p.head.next = cursor
		cursor = p.head
	}
	...
	// if empty or p.head.next = nil
	// just return
	// cursor = nil
	// for loop
	// set tmp = head.next
	// set head.next := cursor
	// update cursor
	// update head := tmp
}
